...
SOURCE = ROOT / "MANUEL_UTILISATEUR_SOFTLIBRARY.md"
OUTPUT = ROOT / "Manuel_utilisateur_SoftLibrary.docx"
LOGO = ROOT.parent.parent / "public" / "softlibrary.png"
...
def cover_page(doc: Document) -> None:
    ...
    title.add_run("Manuel utilisateur SoftLibrary")

    subtitle = doc.add_paragraph(style="Subtitle")
    subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
    subtitle.add_run("Archives physiques, courrier, inventaire, cycle de vie, GED SoftDocs et équipements MFP")

    builder.add_label_value_table(
        doc,
        [
            ("Destinataires", "Utilisateurs finaux, gestionnaires d'archives, agents courrier, validateurs, auditeurs et administrateurs SoftLibrary"),
            ("Périmètre", "Documents physiques, contenants, emplacements, mouvements, consultations, courrier, inventaire, cycle de vie, intégration GED, MFP et administration"),
            ("Version", "1.0"),
            ("Date", "13 juin 2026"),
        ],
    )
    ...
    run = p.add_run("Documentation fonctionnelle rédigée à partir des écrans SoftLibrary")
...
def build_docx() -> None:
    ...
    run = footer.add_run("SoftLibrary - Manuel utilisateur - 13 juin 2026")
    builder.set_run_font(run, size=8.5, color=builder.MUTED)

    doc.core_properties.title = "Manuel utilisateur SoftLibrary"
    doc.core_properties.subject = "Archives physiques, courrier, inventaire, cycle de vie, GED SoftDocs et équipements MFP"
