...
function R12({ docs, tableRef }) {
  ...
  function getMt(p, s, filter) {
    return docs
      .filter(d => d.proj === p && d.site === s && filter(d))
      .reduce((sum, d) => sum + (d.mtR || d.mt || 0), 0);
  }

  const HDR_STYLE = { padding:"8px 10px", fontSize:11, fontWeight:700, color:"#fff", background:"#1d6f42", textAlign:"center", borderRight:"1px solid #16a34a", textTransform:"uppercase", letterSpacing:".04em" };
  const CELL = { padding:"8px 10px", fontSize:12, color:"#212529", borderRight:"1px solid #dee2e6", borderBottom:"1px solid #dee2e6", textAlign:"right" };
  const CELL_L = { ...CELL, textAlign:"left", fontWeight:600 };

  return (
    <div style={{ ...card(), padding:20 }}>
      <div style={{ overflowX:"auto" }}>
        <table ref={tableRef} style={{ width:"100%", borderCollapse:"collapse", border:"1px solid #dee2e6" }}>
          <thead>
            <tr>
              <th style={{ ...HDR_STYLE, textAlign:"left" }}>Projet</th>
              <th style={{ ...HDR_STYLE }}>Site</th>
              <th style={{ ...HDR_STYLE }}>Montant docs reçus</th>
              <th style={{ ...HDR_STYLE }}>Montant docs en cours</th>
              <th style={{ ...HDR_STYLE }}>Montant docs refusés</th>
              <th style={{ ...HDR_STYLE, borderRight:"none" }}>Montant validé</th>
            </tr>
          </thead>
          <tbody>
            {PRJS.length === 0 && (
              <tr><td colSpan={6} style={{ textAlign:"center", padding:"24px", color:MUT }}>Aucune donnée</td></tr>
            )}
            {PRJS.map((proj, pi) => {
              const sites = SITES_BY_PROJ[proj];
              return sites.map((site, si) => (
                <tr key={proj+site} style={{ background: pi % 2 === 0 ? "#f9fbf9" : "#fff" }}
                  onMouseEnter={e => e.currentTarget.style.background = "#ecfdf5"}
                  onMouseLeave={e => e.currentTarget.style.background = pi % 2 === 0 ? "#f9fbf9" : "#fff"}>
                  {si === 0 && (
                    <td rowSpan={sites.length} style={{ ...CELL_L, verticalAlign:"middle", background:"#f0fdf4", fontWeight:800, color:"#1d6f42", borderRight:"2px solid #86efac" }}>
                      {proj}
                    </td>
                  )}
                  <td style={{ ...CELL_L, fontWeight:500 }}>{site}</td>
                  <td style={CELL}>{fmtN(getMt(proj, site, d => true))} Ar</td>
                  <td style={CELL}>{fmtN(getMt(proj, site, d => d.st === "EN VALIDATION"))} Ar</td>
                  <td style={{ ...CELL, color:"#dc3545" }}>{fmtN(getMt(proj, site, d => d.st === "REJETÉ"))} Ar</td>
                  <td style={{ ...CELL, color:"#1d6f42", fontWeight:700, borderRight:"none" }}>{fmtN(getMt(proj, site, d => ["VALIDÉ","BON À PAYER","PAYÉ"].includes(d.st)))} Ar</td>
                </tr>
              ));
            })}
            {/* Totaux */}
            {PRJS.length > 0 && (
              <tr style={{ background:"#1d6f42", fontWeight:800 }}>
                <td colSpan={2} style={{ ...CELL_L, color:"#fff", background:"#1d6f42", borderRight:"2px solid #16a34a" }}>TOTAL GÉNÉRAL</td>
                <td style={{ ...CELL, color:"#fff", background:"#1d6f42", fontWeight:700 }}>{fmtN(docs.reduce((s,d)=>s+(d.mtR||d.mt||0),0))} Ar</td>
                <td style={{ ...CELL, color:"#fff", background:"#1d6f42", fontWeight:700 }}>{fmtN(docs.filter(d=>d.st==="EN VALIDATION").reduce((s,d)=>s+(d.mtR||d.mt||0),0))} Ar</td>
                <td style={{ ...CELL, color:"#fca5a5", background:"#1d6f42", fontWeight:700 }}>{fmtN(docs.filter(d=>d.st==="REJETÉ").reduce((s,d)=>s+(d.mtR||d.mt||0),0))} Ar</td>
                <td style={{ ...CELL, color:"#86efac", background:"#1d6f42", fontWeight:700, borderRight:"none" }}>{fmtN(docs.filter(d=>["VALIDÉ","BON À PAYER","PAYÉ"].includes(d.st)).reduce((s,d)=>s+(d.mtR||d.mt||0),0))} Ar</td>
              </tr>
            )}
          </tbody>
        </table>
...
function R13({ docs, tableRef }) {
  ...
  function getMt(fourn, proj, site, filter) {
    return docs
      .filter(d => d.fourn === fourn && d.proj === proj && d.site === site && filter(d))
      .reduce((sum, d) => sum + (d.mtR || d.mt || 0), 0);
  }

  const HDR = { padding:"8px 10px", fontSize:11, fontWeight:700, color:"#fff", background:"#0891b2", textAlign:"center", borderRight:"1px solid #0e7490", textTransform:"uppercase", letterSpacing:".04em" };
  const CELL = { padding:"8px 10px", fontSize:12, color:"#212529", borderRight:"1px solid #dee2e6", borderBottom:"1px solid #dee2e6", textAlign:"right" };
  const CELL_L = { ...CELL, textAlign:"left", fontWeight:600 };
    ...
    <div style={{ ...card(), padding:20 }}>
      <div style={{ overflowX:"auto" }}>
        <table ref={tableRef} style={{ width:"100%", borderCollapse:"collapse", border:"1px solid #dee2e6" }}>
          <thead>
            <tr>
              <th style={{ ...HDR, textAlign:"left" }}>Fournisseur</th>
              <th style={{ ...HDR }}>Projet</th>
              <th style={{ ...HDR }}>Site</th>
              <th style={{ ...HDR }}>Montant docs reçus</th>
              <th style={{ ...HDR }}>Montant en cours</th>
              <th style={{ ...HDR }}>Montant refusés</th>
              <th style={{ ...HDR, borderRight:"none" }}>Montant validé</th>
            </tr>
          </thead>
          <tbody>
            {FOURNS.length === 0 && (
              <tr><td colSpan={7} style={{ textAlign:"center", padding:"24px", color:MUT }}>Aucune donnée</td></tr>
            )}
            {FOURNS.map((fourn, fi) => {
              const rows = getRows(fourn);
              if (rows.length === 0) return null;
              return rows.map((row, ri) => (
                <tr key={fourn+row.proj+row.site} style={{ background: fi % 2 === 0 ? "#f0fbff" : "#fff" }}
                  onMouseEnter={e => e.currentTarget.style.background = "#e0f7ff"}
                  onMouseLeave={e => e.currentTarget.style.background = fi % 2 === 0 ? "#f0fbff" : "#fff"}>
                  {ri === 0 && (
                    <td rowSpan={rows.length} style={{ ...CELL_L, verticalAlign:"middle", background:"#cffafe", fontWeight:800, color:"#0891b2", borderRight:"2px solid #67e8f9" }}>
                      {fourn}
                    </td>
                  )}
                  <td style={{ ...CELL_L, fontWeight:600, color:"#0891b2" }}>{row.proj}</td>
                  <td style={{ ...CELL_L, fontWeight:400 }}>{row.site}</td>
                  <td style={CELL}>{fmtN(getMt(fourn, row.proj, row.site, d => true))} Ar</td>
                  <td style={CELL}>{fmtN(getMt(fourn, row.proj, row.site, d => d.st === "EN VALIDATION"))} Ar</td>
                  <td style={{ ...CELL, color:"#dc3545" }}>{fmtN(getMt(fourn, row.proj, row.site, d => d.st === "REJETÉ"))} Ar</td>
                  <td style={{ ...CELL, color:"#0891b2", fontWeight:700, borderRight:"none" }}>{fmtN(getMt(fourn, row.proj, row.site, d => ["VALIDÉ","BON À PAYER","PAYÉ"].includes(d.st)))} Ar</td>
                </tr>
              ));
            })}
            {FOURNS.length > 0 && (
              <tr style={{ background:"#0891b2", fontWeight:800 }}>
                <td colSpan={3} style={{ ...CELL_L, color:"#fff", background:"#0891b2", borderRight:"2px solid #0e7490" }}>TOTAL GÉNÉRAL</td>
                <td style={{ ...CELL, color:"#fff", background:"#0891b2", fontWeight:700 }}>{fmtN(docs.reduce((s,d)=>s+(d.mtR||d.mt||0),0))} Ar</td>
                <td style={{ ...CELL, color:"#fff", background:"#0891b2", fontWeight:700 }}>{fmtN(docs.filter(d=>d.st==="EN VALIDATION").reduce((s,d)=>s+(d.mtR||d.mt||0),0))} Ar</td>
                <td style={{ ...CELL, color:"#fca5a5", background:"#0891b2", fontWeight:700 }}>{fmtN(docs.filter(d=>d.st==="REJETÉ").reduce((s,d)=>s+(d.mtR||d.mt||0),0))} Ar</td>
                <td style={{ ...CELL, color:"#a5f3fc", background:"#0891b2", fontWeight:700, borderRight:"none" }}>{fmtN(docs.filter(d=>["VALIDÉ","BON À PAYER","PAYÉ"].includes(d.st)).reduce((s,d)=>s+(d.mtR||d.mt||0),0))} Ar</td>
              </tr>
            )}
          </tbody>
        </table>
