...
function SoftDocsShell(){
  const{view,setView,docs,addDoc,openDoc,selDoc,docCtx,updDoc,authUser,recv,resetAllData}=useApp();
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:99999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:"#fff",borderRadius:12,maxWidth:420,width:"100%",overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,.3)"}}>
            ...
            <div style={{padding:"12px 20px",borderTop:"1px solid #e3e6ea",display:"flex",justifyContent:"flex-end",gap:10,background:"#f8f9fc"}}>
              ...
              <button onClick={()=>{setShowReset(false);}} style={{padding:"8px 16px",borderRadius:6,border:"none",background:"#c0392b",color:"#fff",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>↺ Réinitialiser</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div style={{display:"flex",height:"100vh",overflow:"hidden",background:BG}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
        ...
        <main style={{flex:1,overflowY:"auto",padding:20}}>
          ...
          {view==="users"              && <GestionUsers/>}
          {/* Default */}
          {!["ep-dashboard","ep-liq","ep-liste-paiements","ep-xml","ep-nature-remise","ep-balise-xml","ep-mappage-xml","users","ep-kpi","ep-r-liq","ep-r-gen","ep-r-pai","ep-r-four","ep-r-proj"].includes(view)&&<EPaiementDashboard/>}
        </main>
      </div>
      <ChatBox/>
    </div>
